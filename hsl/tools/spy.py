...
def spy(A, patternonly=True, **kwargs):
    """Plot the sparsity pattern of a sparse matrix object.

    The sparse matrix object must implement `find()`, which must return the
    contents of the matrix in coordinate format, i.e., (val,irow,jcol).
    ...
    if patternonly:
        return fast_spy(nrow, ncol, irow, jcol, sym=sym, **kwargs)
    else:
        return fast_spy(nrow, ncol, irow, jcol, sym=sym, val=values, **kwargs)


def fast_spy(nrow, ncol, irow, jcol, **kwargs):
    """Plot the sparsity pattern of a sparse matrix in coordinate format.
    ...
    if val is not None:
        ...
        absval = np.abs(np.asarray(val))
        s = 100.0 * absval / np.linalg.norm(val, ord=np.infty)
    ...
    if nrow == ncol:
        # ax.set_aspect('equal')
        if t is None:
            t = 'Order %-d with %-d nonzeros' % (nrow, nnz)
    else:
        # ratio = (1.0*nrow)/ncol
        # ax.set_aspect(ratio)
        if t is None:
    ...
    if t is not None:
        t = comment + t
    if showtitle:
        ax.set_title(t, fontsize='x-small')
    return ax
